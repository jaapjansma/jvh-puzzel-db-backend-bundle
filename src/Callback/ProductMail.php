...
class ProductMail {
  ...
  public function selectButtonsCallback($arrButtons, DataContainer $dc) {
    $arrButtons['sendEmail'] = '<button type="submit" name="sendEmail" id="sendEmail" class="tl_submit">' . $GLOBALS['TL_LANG']['tl_jvh_db_member_puzzel_product']['send_email'][0] . '</button>';
    return $arrButtons;
  }

  public function onLoadCallback(DataContainer $dc) {
    if (Input::post('FORM_SUBMIT') == 'tl_select') {
      if (isset($_POST['sendEmail'])) {$container = System::getContainer();
