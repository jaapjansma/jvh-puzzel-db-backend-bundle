...
class CollectionMailController {
  ...
  public function __construct(ContaoFramework $framework = null) {
    ...
    public function sendEmails(int $limit = 25) {
      ...
      $delimiter = ",";
        $objNotificationCollection = \NotificationCenter\Model\Notification::findByType('collection_mail');
        if (null === $objNotificationCollection) {
            return;
        }
        $processedIds = [];
        $db = Database::getInstance();
        $rows = $db->prepare("SELECT * FROM `tl_jvh_db_mail_message` WHERE (`product_id` IS NULL OR `product_id` = 0) ORDER BY `id` ASC")->limit($limit)->execute();
    ...
    public function prepareEmail(\DataContainer $dc) {
      ...
      $objSubjectEnWidget = new \Contao\TextField($arrSubjectEnWidget);
        ...
        $arrMessageNlWidget = \Contao\TextArea::getAttributesFromDca($arrMessageNlField, 'message');
      ...
      $objMessageEnWidget = new \Contao\TextArea($arrMessageEnWidget);
        ...
        if (\Input::post('FORM_SUBMIT') === 'tl_jvh_db_collections_send_email') {
          ...
          $objMessageEnWidget->validate();
            ...
            if ($objSubjectNlWidget->hasErrors() || $objMessageNlWidget->hasErrors() || $objSubjectEnWidget->hasErrors() || $objMessageEnWidget->hasErrors()) {
                $doNotSubmit = true;
            } else {
      ...
      $strBuffer .= '<div class="clr widget">'.$objMessageEnWidget->parse().$this->addFileBrowser('ctrl_message_en').'</div>';
        ...
        if (\Input::post('FORM_SUBMIT') === 'tl_jvh_db_collections_send_email' && !$doNotSubmit) {
            ...
            $db = Database::getInstance();
            $sql = "INSERT INTO `tl_jvh_db_mail_message` (`pid`, `subject`, `msg`, `subject_en`, `msg_en`, `tstamp`) VALUES ";
            $data = [];
            foreach($ids as $id) {
                $data[] = "(" . $connection->quote($id) . ', ' . $connection->quote($values['subject_nl'])  . ', ' . $connection->quote($values['message_nl'])  . ', ' . $connection->quote($values['subject_en'])  . ', ' . $connection->quote($values['message_en'])  . ', ' . time() . ')';
            }
            if (count($data)) {
                $sql .= implode(", ", $data);
                $db->execute($sql);
