  ...
  public function __construct(ContaoFramework $framework = null) {
    ...
    public function prepareEmail(\DataContainer $dc) {
        ...
        if (\Input::post('FORM_SUBMIT') === 'tl_jvh_db_collections_send_email' && !$doNotSubmit) {
            ...
            $db = Database::getInstance();
            $sql = "INSERT INTO `tl_jvh_db_mail_message` (`pid`, `subject`, `msg`, `tstamp`) VALUES ";
            $data = [];
            foreach($ids as $id) {
                $data[] = "(" . $connection->quote($id) . ', ' . $connection->quote($values['subject'])  . ', ' . $connection->quote($values['message'])  . ', '. time() . ')';
            }
            if (count($data)) {
                $sql .= implode(", ", $data);
                $db->execute($sql);
