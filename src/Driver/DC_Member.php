...
class DC_Member extends DC_Table {

  protected string $strAliasTable;

  protected string $strRealTable = 'tl_member';
  ...
  public function __construct($strTable, $arrModule = array())
  {
    ...
    $GLOBALS['TL_DCA'][$strTable]['fields'] = $GLOBALS['TL_DCA'][$this->strRealTable]['fields'];
    foreach($GLOBALS['TL_DCA'][$strTable]['fields'] as $k => $v) {
      unset($GLOBALS['TL_DCA'][$strTable]['fields'][$k]['search']);
      if (in_array($k, ['country', 'collection_count', 'wishlist_count', 'collection_create_date', 'collection_update_date'])) {
        $GLOBALS['TL_DCA'][$strTable]['fields'][$k]['sorting'] = true;
        if (in_array($k, ['collection_count', 'wishlist_count'])) {
          $GLOBALS['TL_DCA'][$strTable]['fields'][$k]['flag'] = 12;
        } elseif (in_array($k, ['collection_create_date', 'collection_update_date'])) {
          $GLOBALS['TL_DCA'][$strTable]['fields'][$k]['flag'] = 12;
        }
      } else {
  ...
  protected function limitMenu($blnOptional = false)
  {
    ...
    $return = parent::limitMenu($blnOptional);
    $this->strTable = $originalTable;
    return $return;
  }
...
<div class="tl_submit_container">
  ...
  public function delete($blnDoNotRedirect=false)
  {
    ...
    $this->Database->prepare("DELETE FROM `tl_jvh_db_collection_status_log` WHERE `pid` IN (SELECT `id` FROM `tl_jvh_db_collection` WHERE `member`=?)")->execute([$this->intId]);
    $this->Database->prepare("DELETE FROM `tl_jvh_db_collection` WHERE `member`=?")->execute([$this->intId]);
    $this->Database->prepare("UPDATE `tl_member` SET `has_collection` = 0, `collection_count` = NULL, `wishlist_count` = NULL, `collection_create_date` = 0, `collection_update_date` = 0 WHERE `id`=?")->execute([$this->intId]);
